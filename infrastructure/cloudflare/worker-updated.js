...
export default {
  async fetch(request, env, ctx) {
    ...
    // Route 1: Proxy /api/cricket/* requests to cricket-agent Cloud Run service
    if (pathname.startsWith('/api/cricket')) {
      try {
    ...
    // Route 2: Serve cricket chatbot at /cricket route and its static assets
    if (pathname === '/cricket' || pathname.startsWith('/cricket/')) {
      try {
        // Redirect to Cloudflare Pages deployment for cricket chatbot
        ...
        console.log('Proxying cricket chatbot to:', targetUrl);
        ...
        console.log('Cricket chatbot response status:', response.status);
        ...
        console.error('Cricket chatbot proxy error:', error);
        return new Response(JSON.stringify({ 
          error: 'Cricket chatbot proxy error', 
          message: error.message,
          targetUrl: targetUrl 
        }), {
          status: 502,
    ...
    // Route 3: Serve ANZX Marketing website at root domain (all other paths)
    try {
      // Redirect to Cloudflare Pages deployment for ANZX Marketing
      const marketingUrl = env.ANZX_MARKETING_URL || 'https://anzx-marketing.pages.dev';
      const targetUrl = `${marketingUrl}${pathname}${url.search}`;
      
      console.log('Proxying marketing site to:', targetUrl);
      ...
      console.log('Marketing site response status:', response.status);
      ...
      console.error('Marketing site proxy error:', error);
      return new Response(JSON.stringify({ 
        error: 'Marketing site proxy error', 
        message: error.message,
        targetUrl: targetUrl 
      }), {
        status: 502,
